...
- [x] 用帧计数做 x 轴的图表，现在只能显示帧数，我希望加入时间显示，在tooltip title上同时显示帧数和时间。因为vmaf json里没有记录视频fps的字段，所以需要用户手动输入。前端topbar在Thresholds左侧加入 FPS [<输入框>]。如果输入fps不合法，则不生效。如果合法，tooltop标题应该是 示例："256 00:04.16"(帧256是第4秒第16帧) "216,001 1:00:00.01"(帧216001=60*3600+1 是整一小时第1帧)（fps=60为例子）(HH:)MM:SS.ff
