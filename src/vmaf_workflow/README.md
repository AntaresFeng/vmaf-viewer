...
当前步骤；点击“取消”或按 `Ctrl+X` 会终止当前下载器或 SSH 子进程，远程阶段
记录为 interrupted/130。
...
都会显示对应的手工 CLI 命令。日志窗口支持鼠标拖选，按 `Ctrl+C` 会将选中文本
复制到系统剪贴板。完成后界面给出 viewer 命令，但不会自动启动服务。
...
- 本机下载器：
  ...
  - `D:\YTDown\yt-dlp.exe`
- 本机可用：`uv`、`scp`、`ssh`
- 本机 `ffprobe`：含 BVID 的项目在 `prepare` 时必需，用于水印识别和真实画面
  几何校验；不含 BVID 的项目仍可在元数据不完整时生成 inventory
- 远端可用：Bash、`tar`、`sha256sum`、Git、FFmpeg、FFprobe
- 远端 FFmpeg 必须包含 `libvmaf` 和 `drawbox`，默认最低主版本是 5
...
- `present`：写入一个归一化 `bilibili_watermark` exclusion；所有后续比较都排除
  同一区域。
- `absent`：继续生成全画面 VMAF。
- `uncertain` 或检测错误：保留诊断并阻断 `prepare`，不写可继续使用的 inventory。

诊断位于 `.workflow/watermark-analysis/`。`summary.json` 记录分析框、归一化框和
每个媒体按真实分辨率换算的审计框；PNG 包括候选覆盖图、频率图和 contact sheet。
水印为 `present` 时，`prepare` 会删除 manifest 中登记的旧全画面
`*_vmaf.json`，避免 Viewer 混用两种评分范围。
...
存在 content exclusion 时，remote-plan 先按 distorted/reference 各自的真实
宽高映射归一化框，再按 easyVmaf 的强制拉伸映射到 HD `1920x1080` 或 4K
`3840x2160`。最终框分别增加 8 或 16 像素安全边距，并为每条命令生成作用于双流
的 `-pre_filter drawbox=...:color=black:t=fill`。`remote-plan.json` 的
`score_scope`、`content_exclusions`、每条命令的 `target_resolution`、
`excluded_bbox` 和 `pre_filter` 共同记录评分范围。
...
开始 preflight 前会重新核对远端 script、package 和 provenance 哈希。upload 的
环境预检还要求 easyVmaf `--help` 包含 `-pre_filter`，并检查 FFmpeg 的
`drawbox` filter。
...
`remote-state.json`，检查关键本地文件、水印 schema，以及小型 inventory/analysis
文件的 SHA-256；不会连接 SSH、重新计算大型归档的 SHA-256 或修改任何状态。
严格的大型内容和哈希校验仍由 package、upload、
...
- yt-dlp 选择器为 `all[height>=1000][vcodec!=none][acodec=none][protocol^=http]`，
  即保留高度至少 1000 像素的纯视频流（包括画面裁切后高度低于 1080 的 1080p 片源），
  且协议以 `http` 开头（https 直链及 DASH 分段），排除 m3u8/HLS 视频流。
...
补下载失效还会删除旧 `.workflow/watermark-analysis/`。重新 `prepare` 检出
水印后，先前登记的全画面 VMAF JSON 会被移除；必须使用新计划重新计算。
