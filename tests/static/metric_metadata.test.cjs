...
const metadata = require("../../src/vmaf_viewer/static/metric_metadata.js");

test("classifies normalized metrics and EGL variants", () => {
  assert.deepEqual(metadata.metricMeta("integer_adm2"), {
    family: "adm",
  ...
  assert.deepEqual(metadata.metricMeta("integer_aim_egl_1"), {
    family: "aim",
    axisGroup: "normalized",
    yAxisIndex: 0,
    axisTag: "0-1",
...
test("classifies raw metrics and rejects model scores or unknown metrics", () => {
  assert.deepEqual(metadata.metricMeta("integer_motion2"), {
    family: "motion",
    axisGroup: "raw",
    yAxisIndex: 1,
    axisTag: "motion",
...
test("toggleDetailMetric enforces Motion and PSNR mutual exclusion", () => {
  assert.deepEqual(
    metadata.toggleDetailMetric(["integer_adm2", "integer_motion2"], "psnr_y"),
    ["integer_adm2", "psnr_y"],
  );
  assert.deepEqual(
